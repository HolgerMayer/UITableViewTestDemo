...
class RegularExpressionValidatorTests: XCTestCase {
    
    let testObject = RegularExpressionValidator()
    
    override func setUp() {
        super.setUp()
        // Put setup code here. This method is called before the invocation of each test method in the class.
        testObject.setRegularExpression(expressionPattern: "(^-?[0-9]*\\.?[0-9]*([eE][+-]?[0-9]*)?$)")
        XCTAssertTrue(testObject.patternOK)
    }
    ...
    func testWrongRegularExpressionPattern(){
        XCTAssertTrue(testObject.patternOK)
        testObject.setRegularExpression(expressionPattern: "([{")
        XCTAssertFalse(testObject.patternOK)
